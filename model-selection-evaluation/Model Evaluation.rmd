...
results <- data.frame(Method = "None", AUC = 0, Sensitivity = 0, Specificity = 0, PPV = 0, NPV = 0,
                      DetectionRate = 0, DetectionPrevalence = 0, BalancedAccuracy = 0, SimpleAccuracy = 0,
...
confusion_matrix <- table(predictions$.pred_class, df$y)
...
ris <- data.frame(Method = "Logistic Regression", AUC = auc,
                  ...
                  DetectionPrevalence = confusion_matrix$byClass["Detection Prevalence"],
...
```{r}
results <- results[-1,]
write.csv(results, file = paste0("Results ",kind))
```
